"""
Programa: visualització i descàrrega d’activacions CNN sota rotació
---------------------------------------------------------------
Entrena una CNN mínima sobre MNIST (sense data‑augmentation),
genera una versió rotada 45° d’un dígit ‘3’ i compara les activacions
de la segona capa convolucional. Guarda la figura i, si s’executa a
Google Colab, l’ofereix per descarregar directament.
"""
...
def plot_and_save(original, rotated, act_orig, act_rot, fname="activacions_rot45.png"):
    fig, ax = plt.subplots(2, 2, figsize=(8, 8))
    ax[0, 0].imshow(original.squeeze(), cmap="gray")
    ax[0, 0].set_title("Original"); ax[0, 0].axis("off")
    ax[0, 1].imshow(rotated.squeeze(), cmap="gray")
    ax[0, 1].set_title("Rotada 45°"); ax[0, 1].axis("off")
    ax[1, 0].imshow(act_orig, cmap="hot")
    ax[1, 0].set_title("Activació Conv2 (orig.)"); ax[1, 0].axis("off")
    ax[1, 1].imshow(act_rot, cmap="hot")
    ax[1, 1].set_title("Activació Conv2 (rot.)"); ax[1, 1].axis("off")
    plt.tight_layout()
...
def main():
    ...
    rotated  = TF.rotate(original.cpu(), angle=45, fill=(0,)).to(device)
    ...
    with torch.no_grad():
        ...
        _ = model(rotated)
        act_rot  = model.activations.squeeze(0).cpu().mean(dim=0).numpy()

    # Figura + desar
    out_file = plot_and_save(original.cpu(), rotated.cpu(), act_orig, act_rot)
