...
def train_model_with_early_stopping(model, optimizer, criterion, train_loader, val_loader, num_epochs, patience,
                                    model_path):
    ...
    for epoch in range(num_epochs):
        ...
        # Verificar si hay una mejora significativa en la pérdida de validación
        if best_val_loss - val_loss > min_delta:
            best_val_loss = val_loss
            patience_counter = 0
            best_model_wts = copy.deepcopy(model.state_dict())
            print(f"Mejora encontrada. Nueva mejor Val Loss: {best_val_loss:.4f}")
        else:
            patience_counter += 1
            print(f"Sin mejora significativa. Contador de paciencia: {patience_counter}/{patience}")
...
def plot_multiple_accuracy_by_angle(accuracy_dicts, labels_list, title, filename):
    ...
    for acc_dict, label in zip(accuracy_dicts, labels_list):
        ...
        # Diferenciar por estilo según si se usó data augmentation o no
        if "con Aug" in label:
            linestyle = '-'  # línea sólida para con data augmentation
        elif "sin Aug" in label:
            linestyle = '--'  # línea discontinua para sin data augmentation
        else:
            linestyle = '-'

        plt.plot(angles, accuracies, linestyle=linestyle, color=color, label=label)
...
def main():
    ...
    if USE_VIT:
        if USE_DATA_AUGMENTATION:
            ...
            if not os.path.exists(model_path_vit_sin):
                print("Entrenando ViT SIN data augmentation con Early Stopping...")
                model_vit_sin = train_model_with_early_stopping(model_vit_sin, opt_vit_sin, crit_vit_sin,
                                                                train_loader_sin_aug, val_loader,
                                                                NUM_EPOCHS, paciencia, model_path_vit_sin)
            else:
            ...
            if not os.path.exists(model_path_vit_con):
                print("Entrenando ViT CON data augmentation con Early Stopping...")
                model_vit_con = train_model_with_early_stopping(model_vit_con, opt_vit_con, crit_vit_con,
                                                                train_loader_con_aug, val_loader,
                                                                NUM_EPOCHS, paciencia, model_path_vit_con)
            else:
    ...
    if USE_VGG:
        if USE_DATA_AUGMENTATION:
            ...
            if not os.path.exists(model_path_vgg_sin):
                print("Entrenando VGG SIN data augmentation con Early Stopping...")
                model_vgg_sin = train_model_with_early_stopping(model_vgg_sin, opt_vgg_sin, crit_vgg_sin,
                                                                train_loader_sin_aug, val_loader,
                                                                NUM_EPOCHS, paciencia, model_path_vgg_sin)
            else:
            ...
            if not os.path.exists(model_path_vgg_con):
                print("Entrenando VGG CON data augmentation con Early Stopping...")
                model_vgg_con = train_model_with_early_stopping(model_vgg_con, opt_vgg_con, crit_vgg_con,
                                                                train_loader_con_aug, val_loader,
                                                                NUM_EPOCHS, paciencia, model_path_vgg_con)
            else:
    ...
    if ROTACION:
        ...
        plot_filename = f"combined_accuracy_linea_{MODEL_ID}.png"
