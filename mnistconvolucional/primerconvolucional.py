...
class PrimerCNN(nn.Module):
    def __init__(self):
        ...
        # Capa convolucional: 1 entrada (grayscale), 10 filtros de 3x3
        self.conv1 = nn.Conv2d(in_channels=1, out_channels=10, kernel_size=tamkernel, stride=1, padding="same")
        # self.conv2 = nn.Conv2d(in_channels=128, out_channels=10, kernel_size=3, stride=1, padding=1)
        self.pool1 = nn.MaxPool2d(kernel_size=2, stride=2)

        # Capa totalmente conectada: reduce de 6272 a 10
        self.fc1 = nn.Linear( 10 * 14 * 14, 100)
        self.fc2 = nn.Linear(100, 10)

    def forward(self, x):
        # Aplicar convolución y activación ReLU
...
filter_evolution = torch.zeros((nummaxepoch, tamkernel, tamkernel))
...
while(sigo and epoch<nummaxepoch):
    ...
    for batch_idx, (images, labels) in enumerate(train_loader):
        ...
        # images=images / 255.0 ya las he normalizado arriba en el transforms.ToTensor() que hace eso
    ...
    for batch_idx2, (images2, labels2) in enumerate(val_loader):
        # Propagación hacia adelante
        # images2 = images2 / 255.0 ya las he normalizado arriba en el transforms.ToTensor() que hace eso
    ...
    first_filter = model.conv1.weight.data[0, 0].clone().detach() # Seleccionamos el primer filtro y su único canal
    # min_val = first_filter.min()
    # max_val = first_filter.max()
    # normalized_filter = (first_filter - min_val) / (max_val - min_val)  # Normalizar entre 0 y 1

    print(epoch)

    # print(filter_evolution[0].shape)

    filter_evolution[epoch]=first_filter
    # filter_evolution[epoch]=normalized_filter

    print(filter_evolution)
    # plt.figure(figsize=(5, 5))
    # plt.imshow(first_filter, cmap='gray')
    # plt.title('Primer Filtro de Conv1')
...
print('Ahora van los filtros')
print(filter_evolution)
# plt.figure(figsize=(15, 5))  # Ajustar el tamaño de la figura para acomodar las imágenes
#
# for i in range(filter_evolution.shape[0]):  # Iterar sobre cada época
#     plt.subplot(1, filter_evolution.shape[0], i + 1)  # Subplot por cada filtro
#     plt.imshow(filter_evolution[i].cpu(), cmap='gray')  # Mostrar la imagen
#     plt.title(f'Época {i+1}')
#     plt.axis('off')  # Ocultar los ejes
#
# plt.tight_layout()  # Ajustar el espacio entre imágenes
# plt.show()

min_global = filter_evolution.min().item()
max_global = filter_evolution.max().item()

num_epochs = filter_evolution.shape[0]
cols = 5  # Queremos 5 epochs por fila
rows = math.ceil(num_epochs / cols)  # Calcula cuántas filas necesitamos

plt.figure(figsize=(15, 3 * rows))
for i in range(filter_evolution.shape[0]):
    plt.subplot(rows, cols, i + 1)

    # 2) normalizamos según min_global y max_global
    normalized = (filter_evolution[i] - min_global) / (max_global - min_global)

    plt.imshow(normalized.cpu(), cmap='gray', vmin=0, vmax=1)
    plt.title(f'Época {i + 1}')
    plt.axis('off')
...
for batch_idx3, (images3, labels3) in enumerate(test_loader):
    # Propagación hacia adelante
    # images3 = images3 / 255.0 ya las he normalizado arriba en el transforms.ToTensor() que hace eso
