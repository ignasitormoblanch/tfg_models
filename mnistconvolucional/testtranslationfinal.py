...
def test_translation(model, test_loader):
    model.eval()
    accuracy_by_translation = {}
    import numpy as np
    with torch.no_grad():
        # Recorremos cada desplazamiento de 0.1 a 5 píxeles en pasos de 0.1
        for trans in np.arange(0.2, 10.2, 0.2):
            trans = round(trans, 1)  # para asegurar un solo decimal
            correct = 0
            total = 0
            for images, labels in test_loader:
                # Aplicamos: padding, traslación (en ambos ejes) y redimensionamiento a 224x224
                images = torch.stack([
                    transforms.functional.resize(
                        transforms.functional.affine(
                            add_padding(img, padding=20),
                            angle=0,
                ...
                total += labels.size(0)
            acc = 100. * correct / total
            accuracy_by_translation[trans] = acc
            # Se imprime el resultado cada 0.5 píxeles
            if abs(trans % 0.5) < 1e-6:
                print(f"Desplazamiento {trans} píxeles - Accuracy: {acc:.2f}%")
    return accuracy_by_translation
...
def plot_multiple_accuracy_by_angle(accuracy_dicts, labels_list, title, filename):
    ...
    plt.ylim(0, 100)  # Fijar el eje y de 0 a 100

    plt.xlabel("Pixeles de translación (°)")
...
def main():
    ...
    if USE_VIT:
            ...
            acc_vit_sin = test_translation(model_vit_sin, test_loader)
    ...
    if USE_VGG:
            ...
            acc_vgg_sin = test_translation(model_vgg_sin, test_loader)
            accuracy_results.append(acc_vgg_sin)
            curve_labels.append("VGG")

    # Graficar todas las curvas en un único gráfico
    if TRANSLACION:
        plot_title = "Precisión vs translación de los Modelos"
        plot_filename = f"combined_accuracy_translacion{MODEL_ID}.png"
        plot_multiple_accuracy_by_angle(accuracy_results, curve_labels, plot_title, plot_filename)
    else:
        print("No se realizó la evaluación con translación.")
